#!/usr/bin/env bash
# Build a one-folder portable PC zip (double-click Start Voyage Chief.bat).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${1:-$ROOT/dist/VoyageChief-PC}"
ZIP="${2:-$ROOT/dist/VoyageChief-PC.zip}"
...
cp "$ROOT/install/pc/Start-NoonReport.bat" "$OUT/Start Voyage Chief.bat"
...
echo "Zip: $ZIP"
echo "Share link (after push/release): download the zip and run Start Voyage Chief.bat"
